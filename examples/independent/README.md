...
that the first program, `left.lp`, marks any predecessors of a node in the
independent set, while the second program, `right.lp`, marks any successors of
a node in the indepent set.

By running

```bash
anthem-cx left.lp right.lp independent.ug
```
...
The modified user guide `independent-assumption.ug` contains the translation of
`assumptions.lp` as obtained by the translation mode of `anthem`. Using this
extended user guide it is possible to verify the equivalence using the
verification mode of `anthem`.
