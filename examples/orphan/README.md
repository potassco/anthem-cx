...
`orphan.1.lp`, derives an auxiliary predicate `parent_living/1` for every
person who has a living parent and then marks every living person without a
living parent as an orphan. The second program, `orphan.2.lp`, directly marks a
living person as an orphan if it has a father and a mother that are both not
living.

The two programs `orphan.1.lp` and `orphan.2.lp` are not externally equivalent.
Running
...
whose parents are not recorded: `orphan.1.lp` considers such a person an
orphan, while `orphan.2.lp` does not, as it requires both a father and a mother
to be present.

## Including assumptions

The program `assumptions.lp` adds the assumption that every living person has
exactly one father and exactly one mother. To check for counterexamples under
these assumptions run
